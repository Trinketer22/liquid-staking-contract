...
export const compile: CompilerConfig = {
    ...
    preCompileHook: async () => {
        ...
        await writeFile(path.join(auto, 'git-hash.tolk'), `const git_hash = 0x${head.trim()};`);

        await writeFile(path.join(auto, 'metadata.tolk'),
          `
const NFT_URI = "${Metadata.NFT_URI}";
const NFT_IMAGE_URI = "${Metadata.NFT_IMAGE_URI}";
          `
          );
    },
